...
train:
	uv run python -m grnti_text_classifier.training.train

evaluate:
	uv run python -m grnti_text_classifier.evaluation.evaluate --checkpoint artifacts/checkpoints/best.ckpt --data data/processed

serve:
	MODEL_PATH=artifacts/checkpoints/best.ckpt uv run uvicorn grnti_text_classifier.serving.main:app --reload --port 8000
test:
	uv run pytest
